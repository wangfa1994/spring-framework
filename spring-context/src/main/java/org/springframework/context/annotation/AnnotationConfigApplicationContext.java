...
public class AnnotationConfigApplicationContext extends GenericApplicationContext implements AnnotationConfigRegistry {
	...
	public AnnotationConfigApplicationContext() { // 进行构造器的调用的时候，会先调用父类的构造器
		...
		this.reader = new AnnotatedBeanDefinitionReader(this); //  beanDefinition的读取器，注解和从资源中读取不是一套体(BeanDefinitionReader) ，在这里会初始化一些注解所需要的后置处理器PostProcessor的beanDefinition,是beanDefinition不是实例 很重要的四个，而且还创建了容器的环境上下文变量
	 ...
	 */
	public AnnotationConfigApplicationContext(Class<?>... componentClasses) {// 这里的前两步只会处理进行添加我们的BeanDefinition，不会设置对应的内置对象实例
		this(); // 创建了底层的容器对象，并且设置了BeanDefinition的读取器以及类扫描器,设置注解所需要的后置处理器的BeanDefinition,classConfigurationPostprocessor,AutowiredAnnotationBeanPostProcessor等
	...
	public void register(Class<?>... componentClasses) {
		...
		this.reader.register(componentClasses); // 利用AnnotatedBeanDefinitionReader进行注册我们的类，变成BeanDefinition,设置到容器中，住进行了配置类的处理，没有处理配置类中的具体信息
