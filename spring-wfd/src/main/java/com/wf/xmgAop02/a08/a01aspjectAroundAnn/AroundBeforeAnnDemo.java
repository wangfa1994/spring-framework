 ...
 * aspectJ的前置动作
 *
 * @Before org.aspectj.lang.annotation.Before  属于aspect 只不过是spring进行了适配
 *
 *  33 aspectJ的前置动作 Before 和 Around 谁先优先执行？
 *
 *
...
public class AroundBeforeAnnDemo {
	...
	//两个AOP， 两个Before ，执行顺序
	private static void beforeOrder() {
	...
	// 一个aop中 Before 和 Around的执行顺序
	private static void beforeAround() {
		AnnotationConfigApplicationContext applicationContext =
				new AnnotationConfigApplicationContext();
		applicationContext.register(AroundBeforeAnnDemo.class, AroundBeforeAnnConfig.class);
		//applicationContext.register(AroundBeforeAnnConfig2.class); // 在增加一个aop，确定执行顺序
