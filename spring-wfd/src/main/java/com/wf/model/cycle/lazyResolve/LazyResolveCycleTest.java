 ...
 */
public class LazyResolveCycleTest {

    public static void main(String[] args) {

		AnnotationConfigApplicationContext annotationConfigApplicationContext = new AnnotationConfigApplicationContext(LazyResolveCycleConfigurations.class);
		//  Husband 的构造器上使用了Lazy，在进行创建的时候，会进行wife的代理对象的创建，然后当使用wife的时候，代理对象才会真正处理对应的依赖关系，然后找到真正的对象进行调用。
		Husband husband = (Husband) annotationConfigApplicationContext.getBean("husband");
		//System.out.println(husband);
		husband.sayWifeHello(); // 此方法中的wife会通过代理对象，找到真正的容器wife对象进行调用

		Wife wife = (Wife) annotationConfigApplicationContext.getBean("wife");
		System.out.println(wife);

		Wife wife1 = husband.getWife();
		System.out.println("husband 中懒加载的wife 是否和容器中的wife相同："+(wife == wife1));

		/** 解决循环依赖的方法
		 * 1. 构造器注入改为setter注入：这是最简单的解决方法之一。Spring在创建bean时，先创建对象，然后再填充属性，这样就能避免循环依赖
		 * 2. 使用@Lazy注解：在其中一个依赖上使用@Lazy注解，Spring会创建一个代理对象，推迟实际bean的创建。
		 * 3. 使用@PostConstruct注解：在bean创建完成后进行依赖注入
		 * 4. 使用ApplicationContextAware接口：实现这个接口，可以在bean创建完成后手动获取依赖
		 * 5. 使用ObjectProvider,ObjectFactory
		 *
		 * 主要目的是延迟加载
		 *
		 *
