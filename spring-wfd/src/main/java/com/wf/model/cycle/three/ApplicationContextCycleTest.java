...
public class ApplicationContextCycleTest {
    ...
    public static void main(String[] args) {
		...
		// B--A C
		// C--A
		// 创建A的时候，实例化之后，先放入到三级缓存singletonFactories中，然后开始处理依赖，发现存在B，
		// 则开始创建B,实例化之后，也放入到三级缓存singletonFactories中，然后开始处理B的依赖，此时发现依赖了A，则开始创建A，此时创建流程则会通过三级缓存得到A的实例，并从三级缓存中移除放入到二级缓存中earlySingletonObjects
		// 然后接着处理依赖C, 开始创建C,实例化之后，放入到三级缓存singletonFactories中，然后开始处理C的依赖，此时发现存在A,则创建A，此时创建流程则会通过二级缓存得到A的实例
		// 最后C属性处理完成，然后将C从二三级缓存移除放到一级缓存中， 退回到B,B属性处理完成，然后将B从二三级缓存移除放到一级缓存中，退会到A，最终A属性处理完成,然后将A从二三级缓存移除放到一级缓存中.
		...
		FirstMan firstMan = (FirstMan) annotationConfigApplicationContext.getBean("firstMan");
 ...
 *
 *
 ** 在创建bean过程中  Initialization 初始化 与 Instantiation 实例化
