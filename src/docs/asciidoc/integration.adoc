...
[source,java,indent=0,subs="verbatim,quotes"]
----
	private MyComponent service;

	public void setMyComponent(MyComponent service) {
		this.service = service;
	}
----
...
`service` property of the controller with the following configuration entry:
...
`service` bean definition creates a proxy for the EJB, which implements the business
...
The `myController` bean definition sets the `service` property of the controller
...
[source,xml,indent=0,subs="verbatim,quotes"]
----
	<jee:local-slsb id="service" jndi-name="ejb/myBean"
...
change the `service` bean definition without changing a line of Java code.
